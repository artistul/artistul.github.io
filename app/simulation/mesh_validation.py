...
def _validation_decision(rows: list[MeshValidationRow], results: list[MeshPipelineResult]) -> dict[str, object]:
    ...
    if trends.get("occupied_cells_change_pct") is None:
        failures.append("Could not compute occupied-cell convergence trend.")
...
def _convergence_trends(rows: list[MeshValidationRow]) -> dict[str, float | None]:
    ...
    if len(ordered) < 2:
        return {"occupied_cells_change_pct": None, "interface_count_change_pct": None, "element_count_ratio": None}
    coarse = ordered[-2]
    fine = ordered[-1]
    occupied_change = _pct_change(coarse.occupied_cells, fine.occupied_cells)
    interface_change = _pct_change(coarse.interface_count, fine.interface_count)
    element_ratio = float(fine.element_count / max(1, coarse.element_count))
    return {
        "occupied_cells_change_pct": occupied_change,
        "interface_count_change_pct": interface_change,
