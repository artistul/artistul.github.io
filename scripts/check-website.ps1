...
Assert-Website ($index -match '2026-06-15-proof-labels-v9' -and $index -match '2026-06-15-fluid-labels-v4' -and $technical -match '2026-06-13-p4') "explicit cache version is missing"
...
Assert-Website ($index -notmatch 'End goal:' -and $index -match '>500 cycles<' -and $index -match '>100 parts<') "numeric proof targets are missing"
Assert-Website ($index -match 'data-current="56"' -and $script -match 'indicatorValue\.textContent') "numeric current indicator behavior is missing"
