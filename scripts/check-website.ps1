...
Assert-Website ($index -match '2026-08-13-sponsor-crowdfunding' -and $technical -match '2026-08-13-sponsor-crowdfunding') "explicit cache version is missing"
Assert-Website ("$index`n$sponsorshipRoute`n$contactRoute" -notmatch 'sponsor-case') "removed sponsor case is still published"
...
Assert-Website (($index -match 'id="tab-achievements"') -and ($sponsorshipRoute -match 'id="tab-achievements"') -and ($contactRoute -match 'id="tab-achievements"') -and ($script -match 'if \(!targetPanel\)')) "achievements navigation is inconsistent across static routes"
...
Assert-Website (($index -match 'https://www\.instagram\.com/volta_circuits/' -and $index -match 'https://www\.tiktok\.com/@volta\.circuits' -and $index -match 'https://volta-circuits-ftc\.com/ro') -and ($sponsorshipRoute -match 'https://www\.instagram\.com/volta_circuits/' -and $sponsorshipRoute -match 'https://www\.tiktok\.com/@volta\.circuits' -and $sponsorshipRoute -match 'https://volta-circuits-ftc\.com/ro') -and ($contactRoute -match 'https://www\.instagram\.com/volta_circuits/' -and $contactRoute -match 'https://www\.tiktok\.com/@volta\.circuits' -and $contactRoute -match 'https://volta-circuits-ftc\.com/ro')) "sponsor social links are incomplete"
Assert-Website (([regex]::Matches("$index`n$sponsorshipRoute`n$contactRoute", 'https://www\.indiegogo\.com/en/projects/influx/influx-injection-molding')).Count -eq 3 -and ([regex]::Matches("$index`n$sponsorshipRoute`n$contactRoute", 'class="sponsor-social sponsor-social-crowdfunding"')).Count -eq 3) "sponsor crowdfunding link is incomplete"
Assert-Website (([regex]::Matches("$index`n$sponsorshipRoute`n$contactRoute", 'class="sponsor-value reveal"')).Count -eq 3 -and ([regex]::Matches("$index`n$sponsorshipRoute`n$contactRoute", 'class="sponsor-value-fact"')).Count -eq 12 -and ([regex]::Matches("$index`n$sponsorshipRoute`n$contactRoute", 'class="sponsor-impact-ledger"')).Count -eq 3 -and "$index`n$sponsorshipRoute`n$contactRoute" -match '1,300\+' -and "$index`n$sponsorshipRoute`n$contactRoute" -match 'You are funding our projects') "sponsor value ledger is incomplete"
Assert-Website (([regex]::Matches("$index`n$sponsorshipRoute`n$contactRoute", 'We bring robotics to people\.')).Count -eq 3) "sponsor presence headline is inconsistent across static routes"
...
Assert-Website (([regex]::Matches($index, 'class="upcoming-entry upcoming-entry--')).Count -eq 9 -and ([regex]::Matches($index, 'data-upcoming-status="qualified"')).Count -eq 3 -and ([regex]::Matches($index, 'data-upcoming-status="scheduled"')).Count -eq 2 -and ([regex]::Matches($index, 'data-upcoming-status="tba"')).Count -eq 4 -and $index -match '<a href="#upcoming-events" aria-current="true"><span>01</span><strong>Upcoming</strong></a>') "upcoming events timeline is incomplete"
